...
foreach ($transclusions as $page) {
	...
	preg_match_all("/\{{2}\s?Rfc(tag)?\s?[^}]*\}{2}/i", $content, $matches);
	...
	preg_match_all("/\{{2}\s?Rfc(tag)?\s?[^}]*\}{2}/i", $content, $match);
	for ($result=0; $result < count($match[0]); $result++) { # For each result on a page
		//Get the details
		
		// Category
		preg_match_all("/\{{2}\s?Rfc(tag)?[^2]\s?[^}]*\}{2}/i", $content, $m);
		...
		if (time() - $timestamp > 2592000 && $timestamp != "" && !preg_match('/<!--\s*RFCBot\s+Ignore\s+Expired\s*-->/i',$content) || preg_match("/\/Archive \d+/", $page)) {
			...
			$content = preg_replace("/\{\{rfc(tag)?\s*(\|[a-z0-9\., ]*)*\s*\|rfcid=$rfcid\s*(\|[a-z0-9\., \|]*)*\s*\}\}(\n|\s)?/i", "", $content);
